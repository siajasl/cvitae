...
const SITE = "markconwaygreenslade.com";
const CONTACT = {
    email: "siajasl@pm.me",
    github: "github.com/siajasl",
    linkedin: "linkedin.com/in/mark-conway-greenslade",
};
...
// PDF document-metadata titles, one per language.
const TITLES = {
    en: "Mark A. Conway-Greenslade — Curriculum Vitae (English)",
    fr: "Mark A. Conway-Greenslade — Curriculum vitae (français)",
    zh: "Mark A. Conway-Greenslade — 简历（中文）",
    de: "Mark A. Conway-Greenslade — Lebenslauf (Deutsch)",
};
// Metadata stamping runs through pikepdf; prefer the repo venv when present.
...
// Employer line with real hyperlinks so the PDF carries link annotations.
function orgMarkup(w) {
    if (w.url) return `<a href="${w.url}">${esc(w.org)}</a>`;
    if (w.links) {
        let out = esc(w.org);
        for (const [name, href] of Object.entries(w.links))
            out = out.replace(esc(name), `<a href="${href}">${esc(name)}</a>`);
        return out;
    }
    return esc(w.org);
}
...
.detail dd{font-size:9px;color:var(--body)}
  ...
  <header class="head">
    ...
    <div class="contact">
      ...
      <a class="c" href="https://${CONTACT.github}">${ICON.github}${esc(CONTACT.github)}</a>
      <a class="c" href="https://${CONTACT.linkedin}">${ICON.linkedin}${esc(CONTACT.linkedin)}</a>
      <span class="c">${esc(d.hero.location)} · <a href="https://${SITE}">${SITE}</a></span>
    </div>
  </header>
...
for (const lang of langs) {
    ...
    // Stamp document metadata and attach the machine-readable CV so AI
    // screeners get structured data alongside the text layer.
    const d = i18n[lang];
    execFileSync(PYTHON, [
        join(ROOT, "scripts", "pdf-metadata.py"),
        out,
        "--title",
        TITLES[lang],
        "--author",
        ...
        d.hero.tagline,
        "--keywords",
        d.skills.flatMap((g) => g.items).join(", "),
        "--lang",
        lang === "zh" ? "zh-CN" : lang,
        "--attach",
        join(ROOT, "assets", "cv.json"),
    ]);
